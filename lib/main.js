		...
		if(KB.db.getPath('/history').translate[selectionText.toLowerCase()]){
			...
			var quijote = Request({
				...
				onComplete: function (response) {
					var out = formatMess('...');
					try{
						console.log('resp',response.text.parse);
						var json = JSON.parse(response.text);
						var out = json.userdict3;
						if(words.length==1){
							KB.db.getPath('/history').translate[selectionText.toLowerCase()] = out;
						}
						//out = res.word_value +' ['+res.transcription+'] ' + '<audio controls><source src="'+res.sound_url+'" type="audio/mpeg">Your browser does not support the audio element.</audio><br>';
						//for(var i in res.translations){
						//	out +=res.translations[i].translate_value + '('+res.translations[i].translate_votes+') ' +'<br>';
						//}
						console.log('out',out);
					}catch(e){
						console.log('err',e);
					...
					if(!(out && out.translations && out.translations.length > 0)){
						console.log("https://translate.yandex.net/api/v1.5/tr.json/translate?key=" + KB.yandex.key + "&lang=" + lang.from+'-'+lang.to + "&text=" + words.join(' '));
						Request({
				            url: "https://translate.yandex.net/api/v1.5/tr.json/translate?key=" + KB.yandex.key + "&lang=" + lang.from+'-'+lang.to + "&text=" + words.join(' '),
				            onComplete: function (response) {
				            	console.log('yandex:',response);
				                var translated = (response && response.json && response.json.text[0]?response.json.text[0]:'');
				                out = {word_top:'',translations:[],word_value:''};
				                out.word_top = translated;
								out.translations.push({translate_value: translated});
								out.word_value = selectionText;
								if(words.length==1) KB.db.getPath('/history').translate[selectionText.toLowerCase()] = out;
								if(!template.isShowing){
									template.show();
								}
