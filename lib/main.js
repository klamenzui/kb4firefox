...
var KB = {
	...
	db: globals.__tree((function(){
		...
		if (!db.storage.settings)db.storage.settings = { language : {def:'en', from:'en', to:'ru',autoDetect:true}, mode:'translate' };
		return db.storage;
	})()
	),
	...
	setLanguage:function(key,lang){
		...
		if(typeof key == 'object'){
			for (var i in key) {
				KB.setLanguage(i,key[i]);
			};
			return;
		}
		lang = ['ru','en'].indexOf(lang)>-1?lang:(KB.db.getPath('/settings/language/'+key,'en'));
		var settLang = KB.db.getPath('/settings/language');
		settLang[key] = lang;
		console.log(key,lang,KB.languages[key]);
		if(KB.languages[key]){
			for(var i in KB.languages[key]){
				console.log(i,key,KB.languages[key][i]);
				KB.languages[key][i].sel = (lang == KB.languages[key][i].val);
			}
			template.port.emit("change_direction",settLang.from,settLang.to);
		}
	},
...
var template = require("sdk/panel").Panel({
	...
	//onHide: handleHide,
	/*contentScript: 'window.onresize = function(event) {'+
		'self.port.emit("log","onresize");'+
		'self.port.emit("resize", {width: document.documentElement.clientWidth, height: document.documentElement.clientHeight});'+
	'};',*/
});
/*template.port.on("resize", function(p){
	console.log('w/h',p.width, p.height);
	template.resize(p.width, p.height);
});*/
...
template.port.on("history_del", function (key) {
	delete KB.db.getPath('/history')[KB.db.getPath('/settings').mode][key];
	template.port.emit("getHistory",KB.db.getPath('/history')[KB.db.getPath('/settings').mode]);
});

template.on("show", function() {
	var settLang = KB.db.getPath('/settings/language');
	console.log(settLang.def);
	template.port.emit("auto_detect",settLang.autoDetect);
	template.port.emit("show",settLang.def);//,formatMess('Simply right-click on selected text and choose "Search etymology".'));
});
...
template.port.on("change_direction", function (lfrom,lto) {
	KB.db.getPath('/settings/language').from = lfrom;
	KB.db.getPath('/settings/language').to = lto;
});
template.port.on("auto_detect", function (val) {
	KB.db.getPath('/settings/language').autoDetect = val;
});
